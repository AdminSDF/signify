...
export default function AdminPage() {
  ...
  const handleRejectWithdrawal = async (requestId: string) => { if (!user?.email) return; try { await updateWithdrawalRequestStatus(requestId, "rejected", user.uid, user.email, "Rejected by admin."); toast({ title: "Withdrawal Rejected" }); fetchAdminData(); } catch (error) { toast({ title: "Rejection Failed", variant: "destructive" }); } };
                                ...
                                <TableCell className="text-right">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            ...
                                            <DropdownMenuItem onClick={() => setEditingUser(u)}><Pencil className="mr-2" />Manage User</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleToggleUserBlock(u.id, u.isBlocked || false)} disabled={u.isAdmin} className={cn(u.isBlocked && "text-green-600 focus:text-green-600")}>{u.isBlocked ? <Users2 className="mr-2" /> : <Ban className="mr-2 text-destructive"/>}{u.isBlocked ? 'Unblock' : 'Block'}</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </TableCell>
                            </TableRow>
      ...
      case 'news-ticker': return (
        <Card><CardHeader><CardTitle>News Ticker</CardTitle><CardDescription>Manage scrolling news items.</CardDescription></CardHeader>
            <CardContent className="space-y-4">
              {currentNewsItems.map((item, index) => (<div key={index} className="flex items-center gap-2">{editingNewsItemIndex === index ? (<><Textarea value={editingNewsItemText} onChange={(e) => setEditingNewsItemText(e.target.value)} className="flex-grow" rows={2}/><Button onClick={handleSaveEditedNewsItem} size="icon" variant="outline"><Save/></Button><Button onClick={() => setEditingNewsItemIndex(null)} size="icon" variant="ghost"><X/></Button></>) : (<><Input value={item} readOnly className="bg-muted" /><Button onClick={() => startEditNewsItem(index)} variant="outline" size="icon"><Edit2/></Button><Button onClick={() => handleRemoveNewsItem(index)} variant="destructive" size="icon"><Trash2/></Button></>)}</div>))}
       ...
       <aside className="fixed inset-y-0 left-0 z-10 hidden w-64 flex-col border-r bg-background sm:flex">
         ...
         <nav className="flex-1 space-y-1 overflow-auto p-2">
          ...
          <div className="mt-auto p-4 border-t space-y-2">
            <Button variant="ghost" className="w-full justify-start" asChild><Link href="/"><Home/>Back to App</Link></Button>
            <Button variant="ghost" className="w-full justify-start text-destructive hover:text-destructive" onClick={() => logout(false)}><LogOut/>Logout</Button>
        </div>
       </aside>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Create New Tournament</DialogTitle></DialogHeader>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          ...
          <div className="space-y-2"><Label>Start Date</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn(!startDate && "text-muted-foreground")}><CalendarIcon/>{startDate ? format(startDate, "PPP") : "Pick date"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={startDate} onSelect={setStartDate} initialFocus /></PopoverContent></Popover></div>
          <div className="space-y-2"><Label>End Date</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn(!endDate && "text-muted-foreground")}><CalendarIcon/>{endDate ? format(endDate, "PPP") : "Pick date"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={endDate} onSelect={setEndDate} initialFocus /></PopoverContent></Popover></div>
